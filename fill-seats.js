...
function fillSeats(seatsConfig, numberOfSeats) {
    ...
    for (i = 0; i < maximumRows; i++) {
        ...
        if (seatsConfig.length == 1) {
            break; // In this case there are no isles.
        }
        ...
        for (j = 1; j <= seatsConfig.length - 2; j++) {
            block = seatsConfig[j];
            blockNo = j;
    ...
    for (i = 0; i < maximumRows; i++) {
        for (j = 0; j <= seatsConfig.length - 1; j++) {

            block = seatsConfig[j];
            blockNo = j;

            if (block[0] > i && block[1] > 2) {
                for (k = 1; k <= block[1] - 2; k++) {
...
// console.log(fillSeats(seatsConfig, 10));